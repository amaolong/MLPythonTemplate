...
1. train three models each with different snp matching method
2. train models with different library (modified forest | random forest with class weight | xgboost | lightgbm)
3. 10 fold cv with roc curve coverage as criterion

'''
def run(_model=None,_matching='tss'):
    '''
    :param _model:
    :param _matching:
    :return:
    ...
    ''' load data '''
    gwava_hgmd_file='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/HGMD_Regulatory/HGMD_Regulatory_found_in_snp147.annotation'
    gwava_tss_file='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/tss/tss.annotation'
    gwava_region_file = '/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/region/region.annotation'
    gwava_unmatched_file='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/unmatched/unmatched.annotation'

    gwava_hgmd_file_fix='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/HGMD_Regulatory/HGMD_Regulatory_found_in_snp147.annotation.fix'
    gwava_tss_file_fix='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/tss/tss.annotation.fix'
    gwava_region_file_fix = '/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/region/region.annotation.fix'
    gwava_unmatched_file_fix='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Annotate/gwava_neg_sets/unmatched/unmatched.annotation.fix'
    #
    outdir='/data/weirauchlab/team/ches2d/MyTools/BedStats++_prediction_based/Predict/output/'
    #
    hgmd=pd.read_csv(gwava_hgmd_file,header=0,sep='\t')
    tss=pd.read_csv(gwava_tss_file,header=0,sep='\t')
    region=pd.read_csv(gwava_region_file,header=0,sep='\t')
    unmatched=pd.read_csv(gwava_unmatched_file,header=0,sep='\t')
    #
    hgmd_fix = pd.read_csv(gwava_hgmd_file_fix, header=0, sep='\t')
    tss_fix = pd.read_csv(gwava_tss_file_fix, header=0, sep='\t')
    region_fix = pd.read_csv(gwava_region_file_fix, header=0, sep='\t')
    unmatched_fix = pd.read_csv(gwava_unmatched_file_fix, header=0, sep='\t')
    # columns to drop
    column_to_replace=['distToNearestTss','distToNearestTes']
    #
    for _ in [hgmd,tss,region,unmatched]:
        _.drop(column_to_replace,axis=1,inplace=True)
    #
    hgmd = hgmd.merge(hgmd_fix[['rsid'] + column_to_replace], how='left', on='rsid')
    tss = tss.merge(tss_fix[['rsid'] + column_to_replace], how='left', on='rsid')
    region = region.merge(region_fix[['rsid'] + column_to_replace], how='left', on='rsid')
    unmatched = unmatched.merge(unmatched_fix[['rsid'] + column_to_replace], how='left', on='rsid')
    #
    tss_data = pd.DataFrame(hgmd.append(tss))
    region_data = pd.DataFrame(hgmd.append(region))
    unmatched_data = pd.DataFrame(hgmd.append(unmatched))
    #
    column_to_drop=['rsid','A_seq','C_seq','G_seq','T_seq']
    #
    tss_data.drop(column_to_drop,axis=1,inplace=True)
    region_data.drop(column_to_drop,axis=1,inplace=True)
    unmatched_data.drop(column_to_drop,axis=1,inplace=True)
    #
    matching_type={'unmatched':unmatched_data,'region':region_data,'tss':tss_data}
    # categorical columns to encode
    column_to_encode=['refAllele','typeOfFirstAltSplicing','typeOfRepeats']
    # encoding categorical columns
    for _ in column_to_encode:
        lbl = LabelEncoder()
        lbl.fit(list(matching_type[_matching][_].values))
        matching_type[_matching][_] = lbl.transform(list(matching_type[_matching][_].values))
    print('data loaded, preprocessing finished')

    ''' model param '''
    # overall class weight, as a dict
    ...
    print('model parameters assigned')
    # 10-fold cv and train/test split
    plot_cv_roc(matching_type[_matching],10,_model,model_params,'label',True,0,roc_curve,_matching,outdir)
...
def plot_cv_roc(df,
                ...
                matching='tss',
                outdir=None):  # may want to change to the one with 1-sd shading,# see http://scikit-learn.org/stable/auto_examples/model_selection/plot_roc_crossval.html#sphx-glr-auto-examples-model-selection-plot-roc-crossval-py
    '''
    :param df:
    :param k:
    :param method:
    :param method_param_dict:
    :param cls:
    :param do_plot:
    :param rs:
    :param curve_type:
    ...
    # X = df.drop(cls,axis=1).as_matrix() only for lgbm
    # model
    #if method == 'forest':
        #clf = RandomForestClassifier(method_param_dict[method])
    if method == 'skrf':
        clf = sk.ensemble.RandomForestClassifier(**method_param_dict[method])
    if method == 'xgb':
        pass
    #if method == 'lgbm':
        #clf = lgbm.LGBMClassifier(method_param_dict[method])
    ...
    for i, (train, test) in enumerate(cv.split(df.drop(cls, axis=1).values,df[cls].values)):
        ...
        if method=='forest' or method=='skrf':
            print('current_cv_',i,' ','train size: ',train.shape,' ','test size: ',test.shape,'\t','test pos label #: ',np.sum(y[test]))
        ...
        #if method=='lgbm':
            #lgbm_data=lgbm.Dataset(X[train].values(), label=y[train].values())
            # form data frame
        # feat importance
    ...
    # estimate feat imps
    if model == 'forest' or model == 'skrf':
        clf.fit(X,y)
    if model == 'xgb':
        xgb_data = xgb.DMatrix(X, label=y)
        clf=xgb.train(method_param_dict[method], xgb_data, num_boost_round=20)
    avg_feat_imps=feat_imp(clf,df,method)
    file_name=outdir+method+'_'+matching+'_feat_imps.txt'
    avg_feat_imps.to_csv(file_name,sep='\t')
...
if __name__ == '__main__':
    ...
    model = sys.argv[1]     # forest, skrf, lgbm, xgb
    matching=sys.argv[2]    # tss, region, unmatched
    #
    run(model,matching)
